...
# # # Forward process, Reverse process and Maximum likelihood loss function 

# # In[4]:


# class DenoiseDiffusion:
#     # eps_model is \epsilon(x_t, t) model, n_steps is t, device is the device to place constants on
#     def __init__(self, eps_model: nn.Module, n_steps: int, device: torch.device):
#         super().__init__()
#         self.eps_model = eps_model 
#         # create \beta_1, ..., \beta_T linearly increasing variance schedule 
#         # beta from 0.0001 to 0.02
#         self.beta = torch.linspace(0.0001, 0.02, n_steps).to(device)
#         # alpha is 1- beta_t
#         self.alpha = 1. - self.beta
#         # alpha_bar = cumulative produce alpha
#         self.alpha_bar = torch.cumprod(self.alpha, dim=0)
#         # time steps T
#         self.n_steps = n_steps
#         # sigma^2 in the denoising density model p(x_{t-1}|x_{t})
#         self.sigma2 = self.beta
        
#         # q(x_t|x_0) distribution
#         def q_xt_x0(self, x0: torch.Tensor, t: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
#             # gather alpha_t and compute the mean \sqrt{alpha_t}x_0, 
#             # input: self.alpha_bar
#             mean = gather(self.alpha_bar, t) ** 0.5 * x0
#             var = 1 - gather(self.alpha_bar, t)
#             return mean, var 
        
#         # forward diffusion process, sample xt
#         # sample x_t from q(x_t|x_0) distribution
#         def q_sample(self, x0: torch.Tensor, t: torch.Tensor, eps: Optional[torch.Tensor] = None):
#             if eps is None:
#                 # \epsilon \sim N(0, 1), first sample true noise
#                 eps = torch.randn_like(x0)
#             # retrive mean and variance from q(x_t|x_0) function
#             mean, var = self.q_xt_x0(x0, t)
#             # sample x_t = mean + sqrt(1-alpha) * eps
#             return mean + (var ** 0.5) * eps
            
#         # reverse denoising process, sample x_t-1
#         # sample from p(x_{t-1}|x_t), derive the mean \mu_theta(x_t, t)
...
#             # \beta / sqrt(1-alpha_bar)
#             eps_coef = (1 - alpha) / (1 - alpha_bar) ** .5
#             # compute the mean \mu_theta(x_t, t) of the denoising model
#             mean = 1 / (alpha ** 0.5) * (xt - eps_coef * eps_theta)
#             # (expandable), we add time embedding to the sigma^2 even through
#             # in this model we set it to be constant. 
#             var = gather(self.sigma2, t)
#             # sample zed which is the variance for x_t-1
#             zed = torch.randn(xt.shape, device=xt.device)
#             # sample from reverse process (denoising + sample)
#             return mean + (var ** .5) * zed
...
#             # sample xt for function q(x_t|x_0)
#             xt = self.q_sample(x0, t, ep=noise)
#             # derive epsilon_theta(xt, t)
#             eps_theta = self.eps_model(xt, t)
#             # compute MSE loss (empirical mean represents the expectation E((epsilon - epsilon_theta)^2) 
